...
ENV VIRTUAL_ENV /opt/Praticando_Mirascope-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/Praticando_Mirascope/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/Praticando_Mirascope/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/praticando_mirascope/ && touch src/praticando_mirascope/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/Praticando_Mirascope/
